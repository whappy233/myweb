...
{% if request.user.is_authenticated %}    # 首先需要登录
    {% if perms.app_blog.add_article %}  # perms.应用名.add_模型名小写  (是否有添加权限)
        do something
    {% endif %}
{% endif %}
'''
...
# 实现方法主要有两种。
# 下面我们将分别使用2种方法给 Article 模型新增了两个权限，一个是 publish_article, 一个是 comment_article。
...
# 比如下例在代码中给用户手动增加了change_blogarticle的权限，如果不重新载入用户，那么将显示用户还是没有change_blogarticle的权限。
...
def user_gains_perms(request, user_id):
    ...
    # 任何权限检查都将缓存当前权限集
    user.has_perm('app_blog.change_article') 
    ...
    # 从数据库重载权限缓存
    user.has_perm('app_blog.change_article')  # True
...
# 例如，如果一个用户组 editors 有权限 change_article , 那么所有属于 editors 组的用户都会有这个权限。
