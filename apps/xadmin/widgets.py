...
# https://blog.csdn.net/bocai_xiaodaidai/article/details/95172133
class M2MFilterWidget(forms.SelectMultiple):
    ...
    def media(self): 
        ...
        # 引入的js css文件名必须是xadmin.widget.xxx.js、xadmin.widget.xxx.css格式，而且要放在xadmin/static/js、xadmin/static/css下面
    ...
    def render(self, name, value, attrs=None):
        ...
        return mark_safe('<div >'
                             f'<div class="m2mfilter" id="m2m_{name}" style="display: none">{choices_data}</div>'
                             f'<div class="col-md-12" style="padding:0">'
                                f'<input type="text" id="selectPage_{name}" class="form-control" name="{name}" value={value_str} placeholder="请输入查询关键字">'
                             '</div>'
                         '</div>')
