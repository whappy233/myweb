...
; (function (window) {
	...
	var currentRoom = 0,
		...
		// Info view transform. 显示Info信息时的过渡
		infoTransform = { translateX: 0, translateY: 0, translateZ: '200px', rotateX: '2deg', rotateY: 0, rotateZ: '4deg' },
		// Room initial moving transition. 房间初始移动过渡
		...
		// How much to rotate when the mouse moves. 鼠标移动时旋转的角度
		tiltRotation = {
			rotateX: 1, // a relative rotation of -1deg to 1deg on the x-axis
			rotateY: -3  // a relative rotation of -3deg to 3deg on the y-axis
		},
		// Transition end event handler.  过渡结束事件处理
		...
		// Check if moving inside the room and check if navigating. 检查是否在房间内移动并检查是否在导航。
	...
	function initEvents() {
			// 鼠标按下移动事件
		var mouseAction = function (ev) {
			if (isMouseDown && !isNavigating) {
					let pos = getMousePos(ev);
					let _x = pos.x, _y = pos.y;
					if (_x - MouseDownPos.x > 300 && !DOM.menuCtrl.classList.contains('btn--active') && !DOM.infoCtrl.classList.contains('btn--active')) {
						isMouseDown = false;
						navigate('prev');
					} else if (_x - MouseDownPos.x < -300 && !DOM.menuCtrl.classList.contains('btn--active') && !DOM.infoCtrl.classList.contains('btn--active')) {
						isMouseDown = false;
						navigate('next');
					} else if (_y - MouseDownPos.y > 300 && !DOM.menuCtrl.classList.contains('btn--active') && !DOM.infoCtrl.classList.contains('btn--active')) {
						isMouseDown = false;
						showMenu();
					} else if (_y - MouseDownPos.y < -300 && DOM.menuCtrl.classList.contains('btn--active') && !DOM.infoCtrl.classList.contains('btn--active')) {
						isMouseDown = false;
		...
		document.addEventListener('mousedown', 	function (e) { isMouseDown = true; MouseDownPos = getMousePos(e) });
		document.addEventListener('mouseup', 	function (e) { isMouseDown = false });


		document.addEventListener('touchmove', 	onMouseMoveFn);
		document.addEventListener('touchstart', function(e){ isMouseDown = true; MouseDownPos = getMousePos(e)  });
