...
ScrollTrigger.addEventListener('scrollEnd', function(){
  ...
  let gallery = BOXES[gsap.utils.wrap(0, BOXES.length, Math.floor(BOXES.length * SCRUB.vars.position))];  // 当前显示的Gallery

  let d = gallery.dataset;
  let cc = d.title ? `<ul style='animation: action 4s infinite;'><li class="title">${d.title}</li><li class="date">${d.time}</li><li class="count">${d.count} 张</li></ul>` : ''

  document.querySelector('.gallery-info').innerHTML = cc;
  document.querySelector('.export a').href = d.href?d.href:'#';
});
